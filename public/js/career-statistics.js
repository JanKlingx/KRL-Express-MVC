(() => {
  ...
  const labels = { points: 'Punkte', starts: 'Starts', wins: 'Siege', poles: 'Polepositions', fastestLaps: 'Schnellste Runden', driverOfTheDays: 'Driver of the Day', winRate: 'Siegesquote' };
  ...
  const seasons = drivers.flatMap(driver => driver.seasons);
  ...
  function metrics(stats) {
    ...
    Object.entries(labels).forEach(([key, label]) => { const item = el('div'); item.append(el('dt', label), el('dd', fmt(stats[key]) + (key === 'winRate' ? ' %' : ''))); list.append(item); });
    return list;
  }
  function render() {
    ...
    const visible = all.filter(driver => driver.name.toLocaleLowerCase('de').includes(query)).sort((a, b) => b.stats[metric] - a.stats[metric] || a.name.localeCompare(b.name, 'de'));
    ...
    visible.forEach(driver => {
      ...
      heading.append(el('h3', driver.name), button); card.append(heading, metrics(driver.stats));
  ...
  get('reset').addEventListener('click', () => { get('search').value = ''; ['discipline', 'league', 'season'].forEach(name => { get(name).value = 'all'; }); get('metric').value = 'points'; selected.clear(); render(); });
  render();
})();
