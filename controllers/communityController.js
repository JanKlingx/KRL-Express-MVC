...
exports.saveNews=async(req,res)=>{
 ...
 try{
  ...
  if(req.file){uploadedImage=await require('../services/imageStorage').saveImage(req.file);body+=`\n\n![Newsbild](${uploadedImage})`;}
 ...
 }catch(error){if(uploadedImage)await require('../services/imageStorage').deleteUpload(uploadedImage);req.session.newsDraft={id:req.params.id||'',title:String(req.body.title||'').slice(0,180),body:String(req.body.body||'').slice(0,20000),isPublished:req.body.isPublished==='on'};req.session.flash={type:'error',message:error.message};}
 res.redirect('/#news');
};
...
exports.saveBranding=async(req,res)=>{
 const {saveImage,deleteUpload}=require('../services/imageStorage');let path;
 try{if(!req.file)throw new Error('Bitte ein Logo als PNG, JPG oder WebP auswählen.');path=await saveImage(req.file);
 const [settings]=await CommunitySetting.findOrCreate({where:{id:1},defaults:{}});await settings.update({logoPath:path});req.session.flash={type:'success',message:'Logo gespeichert.'};
 }catch(error){if(path)await deleteUpload(path);req.session.flash={type:'error',message:error.message};}
 res.redirect('/');
};
