...
async function loadPlanningRows(league, race) {
  const [teams, reserves, entries] = await Promise.all([
    ...
    Driver.findAll({ where: { roleLmuReserve: true }, include: [{ association: 'aliases' }], order: [['name', 'ASC'], ['id', 'ASC']] }),
...
exports.show = async (req, res) => {
  const leagues = await League.findAll({ where: { type: 'lmu' }, order: [['sortOrder', 'ASC'], ['name', 'ASC']] });
  ...
  const activeSeason = selectedLeague ? await Season.findOne({
    where: { leagueType: 'lmu', scopeSlug: selectedLeague.slug, status: 'active' }
  }) : null;
  const races = activeSeason ? await GrandPrixResult.findAll({
    where: { SeasonId: activeSeason.id, LeagueId: selectedLeague.id, discipline: 'lmu' },
  ...
  res.render('admin/lmu-race-lineup', {
    title: 'LMU Fahrereinteilung', leagues, selectedLeague, activeSeason,
...
exports.save = async (req, res) => {
  ...
  if (!race || race.discipline !== 'lmu' || race.seasonRecord?.status !== 'active') {
  ...
  res.redirect(`/admin/lmu-race-lineup?league=${race.LeagueId}&race=${race.id}`);
};
