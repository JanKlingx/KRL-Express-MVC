...
function normalizedF1RoleValues(values) {
  const regularSlugs = Object.entries(REGULAR_ROLE_BY_SLUG)
    .filter(([, field]) => Boolean(values[field])).map(([slug]) => slug);
  return {
    ...values,
    roleF1ReserveFriday: false, roleF1ReserveSaturday: false, roleF1ReserveSunday: false,
    roleFormerF1: !values.roleF1Reserve && regularSlugs.length === 0,
    f1Role: values.roleF1Reserve ? 'reserve' : regularSlugs.length === 1
      ? { freitag: 'friday', samstag: 'saturday', sonntag: 'sunday' }[regularSlugs[0]] : null
  };
}
function driverRoleValuesAfterPromotion(driver, leagueSlug) {
  const field = REGULAR_ROLE_BY_SLUG[leagueSlug];
...
function driverRoleValuesAfterRelease(driver, leagueSlug, reserveSlugs = []) {
  ...
  return normalizedF1RoleValues({
    roleF1Friday: Boolean(driver.roleF1Friday), roleF1Saturday: Boolean(driver.roleF1Saturday),
    roleF1Sunday: Boolean(driver.roleF1Sunday), roleF1Reserve: reserveSlugs.length > 0,
    [field]: false
  });
}
