...
test('Vacant starter appears in a normal attendance card with team logo',async()=>{
 ...
 const d=new JSDOM(html).window.document;const field=d.querySelector('[name="vacantAttendance[d8]"]');assert.equal(field.value,'anwesend');assert.ok(field.closest('.f1-attendance-seat'));assert.equal(field.closest('.f1-attendance-seat').querySelector('img').getAttribute('src'),'/ferrari.png');assert.equal(d.querySelector('.vacant-cockpit-grid'),null);
});
