...
test('Kalender trennt Auswahl und Erstellung; Testtag blendet Rennnummer und Sprint korrekt um', async () => {
  ...
  dom.window.eval(require('node:fs').readFileSync('public/js/calendar-structure.js', 'utf8'));
  ...
  assert.equal(d.querySelector('#calendar-structure').getAttribute('action'), '/admin/f1-calendars/3/structure');
  const toggle = row.querySelector('[data-test-toggle]'); toggle.checked = true; toggle.dispatchEvent(new dom.window.Event('change', { bubbles: true }));
  assert.equal(row.querySelector('[data-round-number-field]').hidden, true);
  ...
  assert.equal(row.querySelector('[data-round-number-field]').textContent, 'R1');
