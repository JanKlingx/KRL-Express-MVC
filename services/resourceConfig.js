...
const number = (name, label, required = false, options = {}) =>
  field(name, label, "number", required, { step: "any", ...options });
const text = (name, label, required = false, options = {}) =>
  field(name, label, "text", required, options);
const textarea = (name, label, required = false, options = {}) =>
  field(name, label, "textarea", required, options);
const date = (name, label, required = false, options = {}) =>
  field(name, label, "date", required, options);
const month = (name, label, required = false, options = {}) =>
  field(name, label, "month", required, options);
const dateTime = (name, label, required = false, options = {}) =>
  field(name, label, "datetime-local", required, options);
const url = (name, label, required = false, options = {}) =>
  field(name, label, "url", required, options);
const checkbox = (name, label, options = {}) =>
  field(name, label, "checkbox", false, options);
const select = (name, label, choices, required = false, options = {}) =>
  field(name, label, "select", required, { choices, ...options });
const relation = (
  name,
  label,
  model,
  ...
  field(name, label, "select", required, {
...
const DRIVER_RANKS = [
  {
    value: "f1-friday",
    label: "Stamm Freitag",
    where: { roleF1Friday: true },
    matches: (driver) => Boolean(driver.roleF1Friday),
  },
  {
    value: "f1-saturday",
    label: "Stamm Samstag",
    where: { roleF1Saturday: true },
    matches: (driver) => Boolean(driver.roleF1Saturday),
  },
  {
    value: "f1-sunday",
    label: "Stamm Sonntag",
    where: { roleF1Sunday: true },
  ...
  {
    value: "lmu-regular",
    label: "LMU Stammfahrer",
    where: { roleLmuRegular: true },
    matches: (driver) => Boolean(driver.roleLmuRegular),
  },
  {
    value: "lmu-reserve",
    label: "LMU Ersatzfahrer",
    where: { roleLmuReserve: true },
    matches: (driver) => Boolean(driver.roleLmuReserve),
  },
  {
    value: "former-f1",
    label: "Ehemalige Formel-1-Fahrer",
    where: { roleFormerF1: true },
    matches: (driver) => Boolean(driver.roleFormerF1),
  },
  {
    value: "former-lmu",
    label: "Ehemalige LMU-Fahrer",
    where: { roleFormerLmu: true },
    matches: (driver) => Boolean(driver.roleFormerLmu),
  },
];
...
async function prepareDriver(values, body, existingDriver) {
  ...
  if (values.TeamId) {
    ...
    if (!team || team.LeagueId !== Number(values.LeagueId))
      throw new Error(
        "Das ausgewählte Team gehört nicht zur ausgewählten Liga.",
      );
  }
  if (values.ParticipatingLeagueId) {
    const participant = await models.ParticipatingLeague.findByPk(
      values.ParticipatingLeagueId,
    );
    if (!participant)
      throw new Error("Die ausgewählte WDL-Liga existiert nicht.");
  }
}
...
const hasLmuRank = (entry) =>
  Boolean(
    entry?.roleLmuRegular || entry?.roleLmuReserve || entry?.roleFormerLmu,
  );
...
const nationalityField = () =>
  select("nationality", "Nationalität", NATIONALITY_CHOICES);
const aliasesField = () =>
...
async function syncDriverAliases(driver, body) {
  ...
  await models.sequelize.transaction(async (transaction) => {
    ...
    if (aliases.length)
      await models.DriverAlias.bulkCreate(
...
async function syncF1Driver(driver, body) {
  ...
  for (const assignment of assignments) {
    ...
    if (roster?.discipline === "f1") {
      const roleField = require("./raceLineup").regularRoleField(
        roster.league?.slug,
      );
      if (!driver[roleField]) await assignment.destroy();
    } else if (
      roster?.discipline === "lmu" &&
      !driver.roleLmuRegular &&
      !driver.roleLmuReserve
    )
...
async function prepareCentralTeam(discipline, values, body, existingTeam) {
  ...
  if (duplicate)
    throw new Error(
      `Das ${discipline === "lmu" ? "LMU" : "Formel-1"}-Team „${values.name}“ existiert bereits.`,
    );
}
...
async function prepareTeamForForm(entry, discipline) {
  ...
  const resultEntries = await models.GrandPrixResultEntry.findAll({
    where: {
      [Op.or]: [{ TeamId: entry.id }, { TeamId: null, teamName: entry.name }],
    },
    attributes: ["points"],
    include: [
      { association: "grandPrixResult", where: { discipline }, attributes: [] },
    ],
  });
  return {
    ...values,
    totalPoints: resultEntries.reduce(
...
async function syncF1Team(team) {
  const participants = await models.ParticipatingLeague.findAll({
    where: { F1TeamId: team.id },
  });
  for (const participant of participants)
    await syncParticipatingLeague(participant);
  return team;
}
...
async function syncCalendarGrandPrix(event) {
  ...
  let grandPrix =
    ...
    (await models.GrandPrixResult.findByPk(event.GrandPrixResultId));
  if (!grandPrix) {
    grandPrix = await models.GrandPrixResult.create({
      LeagueId: league.id,
...
async function removeCalendarGrandPrix(event) {
  ...
  const grandPrix = await models.GrandPrixResult.findByPk(
    event.GrandPrixResultId,
  );
...
async function prepareRaceEntry(values, body, existingEntry) {
  const [race, driver] = await Promise.all([
    ...
    models.Driver.findByPk(body.DriverId, {
      include: [{ model: models.Team, as: "team" }],
    }),
  ]);
  if (!race || !driver)
    throw new Error("Grand Prix und Stammfahrer müssen ausgewählt werden.");
  if (race.LeagueId !== driver.LeagueId)
    throw new Error("Der Stammfahrer gehört nicht zur Liga dieses Grand Prix.");
  const duplicate = await models.GrandPrixResultEntry.findOne({
    where: {
      GrandPrixResultId: race.id,
      [Op.or]: [{ DriverId: driver.id }, { driverName: driver.name }],
    },
  });
  if (duplicate && duplicate.id !== existingEntry?.id)
    throw new Error(
      "Für diesen Stammfahrer existiert bei diesem Grand Prix bereits ein Ergebnis.",
    );
...
async function prepareRaceEntryForForm(entry) {
  ...
  const driver =
    ...
    (await models.Driver.findOne({
      where: { LeagueId: race.LeagueId, name: values.driverName },
    }));
...
async function prepareCockpit(values) {
  ...
  for (const [idField, nameField] of mappings) {
    if (!values[idField]) {
      values[nameField] = null;
      continue;
    }
    const driver = await models.Driver.findByPk(values[idField]);
    const hasRole =
      idField === "ReserveDriverId"
        ? driver?.roleLmuReserve
        : driver?.roleLmuRegular;
    if (!driver || !hasRole)
      throw new Error(
        idField === "ReserveDriverId"
          ? "Der Ersatzfahrer benötigt den Rang „LMU Ersatzfahrer“."
          : "Cockpit-Fahrer benötigen den Rang „LMU Stammfahrer“.",
      );
...
async function prepareWdlStanding(values) {
  ...
  for (const fieldName of ["Driver1Id", "Driver2Id"]) {
    ...
    if (!driver || driver.ParticipatingLeagueId !== participantId)
      throw new Error(
        "Die ausgewählten WDL-Fahrer müssen zur teilnehmenden Liga gehören.",
      );
...
async function prepareSeason(values) {
  ...
  const publicType =
    values.leagueType === "wdl" ? "competition" : values.leagueType;
  const league = await models.League.findOne({
    where: { slug: values.scopeSlug, type: publicType },
  });
  ...
  if (values.PointsSchemeId) {
    ...
    if (!scheme || scheme.discipline !== values.leagueType)
      throw new Error(
        "Das ausgewählte Punktesystem gehört nicht zu dieser Ligaart.",
      );
  }
  if (values.SeasonCategoryId) {
    const category = await models.SeasonCategory.findByPk(
      values.SeasonCategoryId,
    );
    if (
      !category ||
      category.leagueType !== values.leagueType ||
      category.scopeSlug !== values.scopeSlug
    ) {
      throw new Error(
        "Die Saison-Kategorie gehört nicht zum ausgewählten Ligabereich.",
      );
    }
  } else {
    const category = await models.SeasonCategory.findOne({
      where: {
        name: values.status === "active" ? "Aktuelle Saison" : "Ältere Saisons",
...
async function preparePointsScheme(values, body, existingEntry) {
      ...
      [Op.and]: [
        {
          [Op.or]: [
            { validFrom: null },
            { validFrom: { [Op.lte]: values.validUntil || "9999-12-31" } },
          ],
        },
        {
          [Op.or]: [
            { validUntil: null },
            { validUntil: { [Op.gte]: values.validFrom || "0001-01-01" } },
          ],
        },
      ],
    },
  ...
  if (overlaps)
    throw new Error(
      `Der Gültigkeitszeitraum überschneidet sich mit „${overlaps.name}“.`,
    );
...
async function preparePointAllocation(values, body, existingEntry) {
  ...
  if (scheme.discipline !== "f1" && values.raceType === "sprint")
    throw new Error("Sprintpunkte sind ausschließlich für Formel 1 möglich.");
  ...
  if (duplicate && duplicate.id !== existingEntry?.id)
    throw new Error(
      "Für diesen Platz existiert im gewählten Rennen bereits ein Punktewert.",
    );
}
...
async function preparePointsSchemeForList(entry) {
  ...
  return {
    ...values,
    allocations: allocations.map((allocation) => allocation.toJSON()),
  };
}
...
async function prepareSeasonCategory(values) {
  ...
  if (!expectedScopes[values.leagueType]?.includes(values.scopeSlug))
    throw new Error("Kategorie und Ligabereich passen nicht zusammen.");
}
...
async function prepareF1Round(values) {
  const activeSeasons = await models.Season.count({
    where: {
      leagueType: "f1",
      status: "active",
      scopeSlug: { [Op.in]: ["freitag", "samstag", "sonntag"] },
    },
  });
  if (!activeSeasons)
    throw new Error("Lege zuerst mindestens eine aktive Formel-1-Saison an.");
  if (!values.fridayDate && !values.sundayDate)
    throw new Error(
      "Mindestens ein Freitag- oder Sonntagsdatum ist erforderlich.",
    );
}
...
async function prepareSeasonRace(values) {
  ...
  if (!season || !league)
    throw new Error("Saison und Liga müssen ausgewählt werden.");
  if (season.status === "active" && season.calendarMode === "automatic")
    throw new Error(
      "Rennen einer aktiven automatischen Saison werden im jeweiligen Rennkalender angelegt.",
    );
  const expectedType =
    season.leagueType === "wdl" ? "competition" : season.leagueType;
  if (league.type !== expectedType)
    throw new Error("Liga und Saison haben unterschiedliche Ligatypen.");
...
async function prepareSeriesCalendar(values) {
  ...
  const expectedType =
    season.leagueType === "wdl" ? "competition" : season.leagueType;
  if (league.type !== expectedType)
    throw new Error("Der Kalender gehört nicht zum Ligatyp der Saison.");
}

async function prepareSeriesEntry(values, body, existingEntry) {
  const [race, driver] = await Promise.all([
    models.GrandPrixResult.findByPk(values.GrandPrixResultId, {
      include: [{ model: models.Season, as: "seasonRecord" }],
    }),
    models.Driver.findByPk(values.DriverId, {
      include: [{ model: models.Team, as: "team" }],
    }),
  ]);
  if (!race || !driver || race.discipline !== "lmu")
    throw new Error("LMU-Rennen und Fahrer müssen ausgewählt werden.");
  if (race.seasonRecord?.status !== "historical" && !driver.roleLmuRegular) {
    throw new Error(
      "In der aktiven LMU-Saison sind nur LMU-Stammfahrer automatisch zugelassen.",
    );
  }
  const duplicate = await models.GrandPrixResultEntry.findOne({
  ...
  if (duplicate && duplicate.id !== existingEntry?.id)
    throw new Error(
      "Dieser Fahrer besitzt für das Rennen bereits ein Ergebnis.",
    );
  values.driverName = driver.name;
  values.teamName = driver.team?.name || "LMU-Team offen";
  values.points = await pointsForPosition(values.position, {
    ...race.toJSON(),
    fastestLap: values.fastestLap,
    polePosition: values.polePosition,
  });
}
...
async function prepareWdlResult(values, body, existingEntry) {
  ...
  const league = await models.ParticipatingLeague.findByPk(
    values.ParticipatingLeagueId,
    { include: [{ association: "f1Team" }] },
  );
  if (!race || race.discipline !== "wdl" || !league)
    throw new Error("WDL-Rennen und teilnehmende Liga sind erforderlich.");
  if (race.seasonRecord?.status !== "historical" && !league.isActive)
    throw new Error(
      "Inaktive Ligen sind nur in historischen Saisons auswählbar.",
    );
  if (
    values.Driver1Id &&
    values.Driver2Id &&
    Number(values.Driver1Id) === Number(values.Driver2Id)
  )
    throw new Error("Eine Liga muss zwei unterschiedliche Fahrer stellen.");
  const teamDriverIds = new Set(
    (await centralTeamDriverIds(league.F1TeamId)).map(Number),
  );
  for (const driverId of [values.Driver1Id, values.Driver2Id].filter(Boolean)) {
    const driver = await models.Driver.findByPk(driverId);
    if (
      !driver ||
      (Number(driver.ParticipatingLeagueId) !== league.id &&
        !teamDriverIds.has(Number(driver.id)))
    )
      throw new Error(
        "Die ausgewählten Fahrer müssen aus dem zugeordneten F1-Team oder der gewählten WDL-Liga stammen.",
      );
  }
  const duplicate = await models.WdlResultEntry.findOne({
    where: { GrandPrixResultId: race.id, ParticipatingLeagueId: league.id },
  });
  if (duplicate && duplicate.id !== existingEntry?.id)
    throw new Error("Für diese Liga existiert bereits eine Ergebniszeile.");
  values.pointsOne = await pointsForPosition(values.positionOne, {
    ...race.toJSON(),
    fastestLap: values.fastestLapOne,
  });
  values.pointsTwo = await pointsForPosition(values.positionTwo, {
    ...race.toJSON(),
    fastestLap: values.fastestLapTwo,
  });
...
async function syncParticipatingLeague(participant) {
  ...
  const activeSeason = await models.Season.findOne({
    where: { leagueType: "wdl", scopeSlug: "wettkampf", status: "active" },
  });
  if (!activeSeason) return;
  const races = await models.GrandPrixResult.findAll({
    where: { SeasonId: activeSeason.id, discipline: "wdl" },
  });
  if (!races.length) return;
  for (const race of races) await ensureWdlEntries(race);
  const freshParticipant = await models.ParticipatingLeague.findByPk(
    participant.id,
    { include: [{ association: "f1Team" }] },
  );
  ...
  const entries = await models.WdlResultEntry.findAll({
    where: {
      ParticipatingLeagueId: participant.id,
      GrandPrixResultId: { [Op.in]: races.map((race) => race.id) },
    },
  });
...
async function prepareParticipatingLeague(values) {
  ...
  const team = await models.Team.findOne({
    where: { id: values.F1TeamId, LeagueId: null, discipline: "f1" },
  });
  ...
  if (driverIds.length < 2)
    throw new Error(
      `${team.name} benötigt zuerst eine vollständige F1-Aufstellung mit mindestens zwei Fahrern.`,
    );
}

async function prepareKrlAssignment(values, body, existingEntry) {
  if (Number(values.KrlTeamId) < 1 || Number(values.DriverId) < 1)
    throw new Error("Team und Fahrer sind erforderlich.");
  const duplicate = await models.KrlTeamAssignment.findOne({
    where: {
      KrlTeamId: values.KrlTeamId,
      DriverId: values.DriverId,
      roleName: values.roleName,
    },
  });
  if (duplicate && duplicate.id !== existingEntry?.id)
    throw new Error("Diese Fahrer-Rolle ist im Team bereits vorhanden.");
}

async function prepareKrlTeam(values, body, existingEntry) {
  values.name = String(values.name || "").trim();
  if (!values.name) throw new Error("Der Teamname ist erforderlich.");
  if (!/^#[0-9a-f]{6}$/i.test(values.accentColor || ""))
    throw new Error("Bitte eine gültige Teamfarbe auswählen.");
  if (existingEntry?.slug) values.slug = existingEntry.slug;
    ...
    let slug = base;
    let suffix = 2;
    while (await models.KrlTeam.findOne({ where: { slug } })) {
      slug = `${base}-${suffix}`;
      suffix += 1;
    }
    values.slug = slug;
...
async function prepareF1Track(values) {
  ...
  if (!country)
    throw new Error("Bitte ein Land aus dem Länderstamm auswählen.");
...
async function prepareHistoricF1Team(values) {
  const baseTeam = await models.Team.findOne({
    where: {
      id: Number(values.BaseTeamId),
      LeagueId: null,
      discipline: "f1",
    },
  });
  ...
  if (!baseTeam)
    throw new Error(
      "Bitte ein aktuelles Formel-1-Team als Verknüpfung auswählen.",
    );

  values.name = String(values.name || "").trim();

  if (!values.name)
    throw new Error("Der historische Teamname ist ein Pflichtfeld.");

  if (!/^#[0-9a-f]{6}$/i.test(values.accentColor || ""))
    throw new Error("Bitte eine gültige Fahrzeugfarbe auswählen.");
}

const penaltyRuleResource = (discipline, title, group) => ({
  title,
  group,

  description:
    "Strafarten und Strafpunkte pflegen. Neue Strafarten können hier ohne Codeänderung angelegt werden.",

  model: models.PenaltyRule,
  ...
  prepareValues: async (values) => {
    ...
    if (!values.label) {
      throw new Error("Bitte eine Bezeichnung für die Strafregel eintragen.");
    }

    if (!values.status) {
      throw new Error("Bitte einen technischen Schlüssel eintragen.");
    }
  },

  getCreateDefaults: async () => ({
    points: 0,
    isActive: true,
    sortOrder: 0,
  }),

  listFields: ["label", "status", "points", "isActive", "sortOrder"],

  fields: [
    text("label", "Bezeichnung", true, {
      placeholder: "z. B. Gefährliches Zurückfahren auf die Strecke",
      help: "Sichtbarer Name der Strafregel.",
    }),

    text("status", "Technischer Schlüssel", true, {
      placeholder: "z. B. unsafe-rejoin",
      help: "Eindeutiger interner Schlüssel. Für automatische Regeln muss dieser Schlüssel zur Auslöser-Logik passen.",
    }),

    number("points", "Strafpunkte", true, {
      min: 0,
      step: 1,
    }),

    checkbox("isActive", "Regel aktiv"),

    number("sortOrder", "Reihenfolge", false, {
      min: 0,
      step: 1,
    }),
  ],
...
module.exports = {
  statistics: {
    ...
    group: "Frontend",
    description: "Kennzahlen auf der Startseite verwalten.",
    model: models.SiteStatistic,
    fields: [
      text("key", "Technischer Schlüssel", true, {
        help: "Einmaliger kurzer Name, z. B. aktive-fahrer.",
      }),
      text("label", "Bezeichnung", true),
      text("value", "Angezeigter Wert", true),
      text("icon", "Symbol", false, { placeholder: "🏁" }),
      number("sortOrder", "Reihenfolge", false, { min: 0 }),
    ],
  },
  teamCategories: {
    title: "Team-Kategorien",
    group: "Startseite & Team",
    description: "Legacy-Teamkategorien.",
    model: models.TeamCategory,
    hidden: true,
    fields: [
      text("name", "Name", true),
      text("slug", "Kurzname für die URL", true, {
        help: "Kleinbuchstaben ohne Leerzeichen, z. B. rennleitung.",
      }),
      number("sortOrder", "Reihenfolge", false, { min: 0 }),
    ],
  },
  teamMembers: {
    ...
    group: "Startseite & Team",
    description: "Legacy-Teammitglieder.",
    model: models.TeamMember,
    upload: { field: "imagePath", label: "Personenbild" },
    hidden: true,
    fields: [
      relation(
        "TeamCategoryId",
      ...
      text("name", "Name", true),
      text("role", "Funktion", true),
      number("joinedYear", "Eintrittsjahr", false, { min: 2000, step: 1 }),
      number("sortOrder", "Reihenfolge", false, { min: 0 }),
    ],
  },
  leagues: {
    ...
    fields: [
      text("name", "Name", true),
      text("slug", "URL-Kurzname", true, {
        help: "Beispiel: freitag oder sonntag.",
      }),
      select(
        "type",
        "Ligatyp",
        [
          ...
          ["competition", "Wettkampf der Ligen"],
          ["endurance", "Endurance"],
        ],
        true,
      ),
      text("currentSeason", "Aktuelle Saison", true, {
        placeholder: "Saison 12",
      }),
      text("raceDay", "Renntag"),
  ...
  pointsRules: {
    title: "Punktetabelle",
    group: "Stammdaten",
    description: "Legacy-Punktetabelle für bestehende Installationen.",
    model: models.PointsRule,
    afterSave: recalculateAllPoints,
    afterRemove: recalculateAllPoints,
    hidden: true,
    fields: [
  ...
  pointsSchemes: {
    ...
    group: "Liga-Stammdaten",
    description:
      "Punktesystem und zugehörige Punkte je Platz gemeinsam verwalten. Hauptrennen und F1-Sprints sind direkt am jeweiligen System aufgelistet.",
    model: models.PointsScheme,
    prepareValues: preparePointsScheme,
    prepareEntry: preparePointsSchemeForList,
    afterSave: recalculateAllPoints,
    afterRemove: recalculateAllPoints,
    ...
    fields: [
      ...
      date("validFrom", "Gültig von"),
      date("validUntil", "Gültig bis"),
      checkbox("fastestLapEnabled", "Punkte für schnellste Runde"),
      number("fastestLapPoints", "Punkte für schnellste Runde", false, {
        min: 0,
        step: 0.5,
      }),
      checkbox("polePositionEnabled", "LMU: Punkte für Poleposition"),
      number("polePositionPoints", "LMU: Punkte für Poleposition", false, {
        min: 0,
        step: 0.5,
      }),
  ...
  pointAllocations: {
    title: "Punkte je Platz",
    group: "Stammdaten",
    description:
      "Punktewert direkt innerhalb des ausgewählten Punktesystems ergänzen oder bearbeiten.",
    model: models.PointAllocation,
    prepareValues: preparePointAllocation,
    afterSave: recalculateAllPoints,
    afterRemove: recalculateAllPoints,
    getCreateDefaults: (req) => ({ PointsSchemeId: req.query.scheme || "" }),
    returnHref: "/admin/pointsSchemes",
    hidden: true,
    fields: [
      relation(
        "PointsSchemeId",
        "Punktesystem",
        models.PointsScheme,
        (row) => `${row.name} · ${row.discipline.toUpperCase()}`,
        true,
      ),
      select(
        "raceType",
  ...
  seasonCategories: {
    title: "Saison-Kategorien",
    group: "Saisonverwaltung",
    description:
      "Saisons übersichtlich unter Kategorien wie „Aktuelle Saison“ oder „Ältere Saisons“ gruppieren.",
    model: models.SeasonCategory,
  ...
  seasons: {
    ...
    group: "Saisonverwaltung",
    description:
      "Aktuelle und historische Saisons für Frontend, Kalender, Results und Wertungen verwalten.",
    model: models.Season,
    prepareValues: prepareSeason,
    afterSave: syncSeason,
    hidden: true,
    fields: [
      text("name", "Saisonname", true, {
        placeholder: "2024, Season 8 oder WDL 2023",
      }),
      select(
        "leagueType",
        "Ligatyp",
      ...
      select(
        ...
        "Rennkalender-Modus",
        [
          ["automatic", "Automatisch aus Stammdaten"],
          ["manual", "Manuell pflegen"],
        ],
        true,
      ),
      field("accentColor", "Saisonfarbe", "color", false, {
        help: "Ersetzt in dieser Saison die allgemeine Ligafarbe.",
      }),
      relation(
        "PointsSchemeId",
        "Punktesystem",
      ...
      relation(
        "SeasonCategoryId",
        "Kategorie",
        models.SeasonCategory,
        (row) => `${row.name} · ${row.scopeSlug}`,
      ),
      number("sortOrder", "Reihenfolge", false, { min: 0 }),
    ],
  },
  ...
  drivers: {
    ...
    beforeRemove: removeF1Driver,
    rankFilters: DRIVER_RANKS,
    groupByRanks: true,
    fields: [
      text("name", "Name", true),
      checkbox("confirmDuplicateName", "Namensgleichheit bestätigt", {
        persist: false,
      ...
      number("racesF1", "Gefahrene Rennen F1", false, {
        ...
        readonly: true,
        persist: false,
        visibleWhen: hasF1Rank,
      }),
      number("racesLmu", "Gefahrene Rennen LMU", false, {
        min: 0,
        step: 1,
        readonly: true,
        persist: false,
        visibleWhen: (entry) =>
          entry?.roleLmuRegular ||
      ...
      number("winsF1", "Siege F1", false, {
        readonly: true,
        persist: false,
        visibleWhen: hasF1Rank,
      }),
      number("winRateF1", "Siegesquote F1 (%)", false, {
        readonly: true,
        persist: false,
        visibleWhen: hasF1Rank,
      }),
      number("podium1F1", "Platz 1 F1", false, {
        readonly: true,
        persist: false,
        visibleWhen: hasF1Rank,
      }),
      number("podium2F1", "Platz 2 F1", false, {
        readonly: true,
        persist: false,
        visibleWhen: hasF1Rank,
      }),
      number("podium3F1", "Platz 3 F1", false, {
        readonly: true,
        persist: false,
        visibleWhen: hasF1Rank,
      }),
      number("pointsLmu", "LMU-Punkte", false, {
        readonly: true,
        persist: false,
        visibleWhen: (entry) =>
          entry?.roleLmuRegular ||
      ...
      number("winsLmu", "Siege LMU", false, {
        readonly: true,
        persist: false,
        visibleWhen: (entry) =>
          entry?.roleLmuRegular ||
      ...
      number("winRateLmu", "Siegesquote LMU (%)", false, {
        readonly: true,
        persist: false,
        visibleWhen: (entry) =>
          entry?.roleLmuRegular ||
      ...
      number("podium1Lmu", "Platz 1 LMU", false, {
        readonly: true,
        persist: false,
        visibleWhen: (entry) =>
          entry?.roleLmuRegular ||
      ...
      number("podium2Lmu", "Platz 2 LMU", false, {
        readonly: true,
        persist: false,
        visibleWhen: (entry) =>
          entry?.roleLmuRegular ||
      ...
      number("podium3Lmu", "Platz 3 LMU", false, {
        readonly: true,
        persist: false,
        visibleWhen: (entry) =>
          entry?.roleLmuRegular ||
  ...
  lmuCars: {
    title: "LMU-Autos & Marken",
    group: "LMU Stammdaten",
    description:
      "LMU-Fahrzeuge einmalig als Stammdaten pflegen und anschließend direkt den LMU-Stammfahrern zuordnen.",
    model: models.LmuCar,
    ...
    fields: [
      ...
      text("vehicleClass", "Klasse", false, {
        placeholder: "Hypercar, LMP2 oder LMGT3",
      }),
      text("additionalInfo", "Zusatz", false, {
        placeholder: "Optionaler Hinweis, Variante oder Baujahr",
      }),
    ],
  },
  f1CarProfiles: {
    ...
    group: "Formel 1 Stammdaten",
    description:
      "Historischen Teamnamen, Farbe und Logo pflegen und dem heutigen Stammteam zuordnen, z. B. Renault → Alpine.",
    model: models.F1CarProfile,
    upload: { field: "logoPath", label: "Fahrzeug-/Teamlogo" },
    prepareValues: prepareHistoricF1Team,
    listFields: ["BaseTeamId", "name", "accentColor"],
    cardView: "historic-f1-teams",
    nextHref: "/admin/season-setup",
    nextLabel: "Danach einer Saison zuweisen",
    fields: [
      relation(
        "BaseTeamId",
        "Zugehöriges aktuelles Formel-1-Team",
        models.Team,
        (row) => row.name,
        true,
        { where: { LeagueId: null, discipline: "f1" } },
      ),
  ...
  f1Tracks: {
    ...
    group: "Formel 1 Stammdaten",
    description:
      "Strecken einmalig pflegen. Land und Flagge kommen automatisch aus dem Länderstamm.",
    model: models.F1Track,
    prepareValues: prepareF1Track,
    listFields: ["CountryId", "name"],
    fields: [
  ...
  f1RuleSections: {
    ...
    group: "Formel 1 Stammdaten",
    description:
      "Regelwerk und Strafenkatalog für Freitag, Samstag und Sonntag strukturiert veröffentlichen.",
    model: models.F1RuleSection,
    ...
    fields: [
      select(
        ...
        [
          ["rule", "Regelwerk"],
          ["penalty", "Strafenkatalog"],
        ],
        true,
      ),
      text("title", "Überschrift", true),
  ...
  lmuDrivers: {
    title: "LMU-Fahrer",
    group: "LMU – Stammdaten",
    description: "LMU-Fahrer aus den Stammdaten Teams zuordnen.",
    model: models.Driver,
    filterByLeague: true,
    getListWhere: listWhereForLeagueType("lmu"),
    upload: { field: "avatarPath", label: "Fahrerbild" },
    prepareValues: prepareDriver,
    prepareEntry: prepareDriverForForm,
    afterSave: syncDriverAliases,
    hidden: true,
    fields: [
      relation("LeagueId", "LMU-Liga", models.League, (row) => row.name, true, {
        where: { type: "lmu" },
      }),
      relation("TeamId", "LMU-Team", models.Team, (row) => row.name),
      text("name", "Fahrername", true),
      aliasesField(),
      platformField(),
      nationalityField(),
      text("car", "Fahrzeug"),
  ...
  wdlDrivers: {
    title: "WDL-Fahrer",
    group: "Wettkampf der Ligen",
    description: "WDL-Fahrer einer teilnehmenden Liga zuordnen.",
    model: models.Driver,
    getListWhere: listWhereForLeagueType("competition"),
    upload: { field: "avatarPath", label: "Fahrerbild" },
    prepareValues: prepareDriver,
    prepareEntry: prepareDriverForForm,
    afterSave: syncDriverAliases,
    ...
    fields: [
      ...
      relation(
        "ParticipatingLeagueId",
        "Teilnehmende Liga / WDL-Team",
        models.ParticipatingLeague,
        (row) =>
          row.abbreviation ? `${row.name} (${row.abbreviation})` : row.name,
        true,
      ),
      text("name", "Fahrername", true),
      aliasesField(),
  ...
  gpResults: {
    title: "Grand Prix & Rennposter",
    group: "F1 – Saisonverlauf",
    description:
      "Zuerst ein Rennen anlegen und optional das Rennposter hochladen.",
    model: models.GrandPrixResult,
    upload: { field: "imagePath", label: "Rennposter" },
    hidden: true,
    nextResource: "gpResultEntries",
    nextLabel: "Danach Fahrer klassifizieren",
    fields: [
      relation("LeagueId", "Liga", models.League, (row) => row.name, true, {
        where: { type: "f1" },
      }),
      text("season", "Saison", true),
      text("title", "Grand Prix", true),
      text("circuit", "Strecke"),
      date("raceDate", "Renndatum"),
      number("sortOrder", "Rennrunde", false, {
        min: 1,
        step: 1,
        help: "Bestimmt die Reihenfolge im Ergebnis-Karussell.",
      }),
    ],
  },
  gpResultEntries: {
    title: "Saisonverlauf eintragen",
    group: "F1 – Saisonverlauf",
    description:
      "Hier pro Rennen die Stammfahrer, Positionen und Punkte erfassen. GP-Results sowie Fahrer- und Team-WM werden daraus automatisch erzeugt.",
    model: models.GrandPrixResultEntry,
    ...
    fields: [
      relation(
        "GrandPrixResultId",
        "Grand Prix",
        models.GrandPrixResult,
        (row) => `${row.season} · ${row.title}`,
        true,
      ),
      number("position", "Platz", false, {
        min: 1,
        step: 1,
        help: "Bei DNS/DNQ kann das Feld leer bleiben.",
      }),
      relation(
        "DriverId",
        "Stammfahrer",
        models.Driver,
        (row) => `#${row.id} · ${row.name} · ${row.platform}`,
        true,
      ),
      ...
      select(
        ...
        "Rennstatus",
        [
          ["", "Gewertet / Zieleinlauf"],
          ["DNF", "DNF – nicht beendet"],
          ["DNS", "DNS – nicht gestartet"],
          ["DNQ", "DNQ – nicht qualifiziert"],
          ["DSQ", "DSQ – disqualifiziert"],
          ["DNA", "DNA – nicht angetreten"],
        ],
        false,
      ),
  ...
  f1CalendarRounds: {
    title: "F1-Rennkalender (aktuell)",
    group: "Formel 1 Liga",
    description:
      "Eine Strecke pflegen; Freitag und Sonntag werden automatisch als getrennte Rennen erzeugt.",
    model: models.F1CalendarRound,
    ...
    fields: [
      ...
      date("sundayDate", "Datum Sonntag"),
      date("fridayDate", "Datum Freitag"),
      checkbox("hasSprint", "Sprint-Event"),
      checkbox("isTestDay", "Als Testtag markieren"),
      text("sundayTime", "Startzeit Sonntag", false, { placeholder: "19:00" }),
      text("fridayTime", "Startzeit Freitag", false, { placeholder: "19:30" }),
      number("sortOrder", "Rennrunde", false, { min: 1, step: 1 }),
    ],
  },
  seasonRaces: {
    title: "Historische / manuelle Rennen",
    group: "Saisonverwaltung",
    description:
      "Rennen für historische Saisons oder manuelle Kalender anlegen.",
    model: models.GrandPrixResult,
    prepareValues: prepareSeasonRace,
    afterSave: syncSeasonRace,
    afterRemove: recalculateDriverRaceCounts,
    hidden: true,
    fields: [
      relation(
        "SeasonId",
      ...
      text("title", "Rennen / GP", true),
      text("circuit", "Strecke", true),
      date("raceDate", "Datum", true),
      select(
        "raceType",
        "Rennentyp",
        [
  ...
  lmuSeasonCalendar: {
    title: "LMU-Rennkalender",
    group: "LMU",
    description: "Aktuelle LMU-Strecken mit Datum und Startzeit pflegen.",
    model: models.RaceEvent,
    prepareValues: prepareSeriesCalendar,
    afterSave: syncSeriesCalendarEvent,
    beforeRemove: removeSeriesCalendarEvent,
    cardView: "calendar-series",
    hidden: true,
    fields: [
      relation(
        "SeasonId",
  ...
  wdlSeasonCalendar: {
    title: "WDL-Rennkalender",
    group: "WDL",
    description: "Aktuelle WDL-Strecken mit Datum und Startzeit pflegen.",
    model: models.RaceEvent,
    prepareValues: prepareSeriesCalendar,
    afterSave: syncSeriesCalendarEvent,
    beforeRemove: removeSeriesCalendarEvent,
  ...
  lmuResultEntries: {
    title: "LMU-Saisonverlauf",
    group: "LMU",
    description:
      "LMU-Platzierungen eintragen; Punkte, WM, GP-Results und Rennanzahl werden automatisch berechnet.",
    model: models.GrandPrixResultEntry,
    prepareValues: prepareSeriesEntry,
    afterSave: syncSeriesEntry,
    afterRemove: recalculateDriverRaceCounts,
    hidden: true,
    fields: [
      relation(
      ...
      number("points", "Punkte (automatisch)", false, {
        readonly: true,
        persist: false,
      }),
      select(
  ...
  wdlResultEntries: {
    title: "WDL-Saisonverlauf",
    group: "WDL",
    description:
      "Zwei Fahrerplätze je Liga eintragen; Punkte, Liga-Standings, Results und Diagramm entstehen automatisch.",
    model: models.WdlResultEntry,
    prepareValues: prepareWdlResult,
    afterSave: syncWdlResult,
    hidden: true,
    fields: [
      relation(
        "GrandPrixResultId",
      ...
      number("positionOne", "Platz Fahrer 1", false, { min: 1, step: 1 }),
      ...
      number("positionTwo", "Platz Fahrer 2", false, { min: 1, step: 1 }),
      checkbox("fastestLapTwo", "Schnellste Runde Fahrer 2"),
      number("totalPoints", "Gesamtpunkte (automatisch)", false, {
        readonly: true,
        persist: false,
      }),
      number("sortOrder", "Reihenfolge", false, { min: 0 }),
  ...
  cockpits: {
    ...
    group: "LMU",
    description:
      "Fahrzeuge und Fahrerbesetzungen aus den LMU-Stammdaten auswählen.",
    model: models.LmuCockpit,
    upload: { field: "logoPath", label: "Cockpit-/Teamlogo" },
    prepareValues: prepareCockpit,
    hidden: true,
    fields: [
      relation("LeagueId", "LMU-Liga", models.League, (row) => row.name, true, {
        where: { type: "lmu" },
      }),
      text("teamName", "Teamname", true),
      text("car", "Fahrzeug"),
      text("vehicleClass", "Klasse"),
      text("carNumber", "Startnummer"),
      relation(
        "Driver1Id",
        "Fahrer 1",
        models.Driver,
  ...
  f1Calendar: {
    title: "F1-Rennkalender",
    group: "Rennkalender",
    description:
      "Rennen anlegen und anschließend direkt den tabellarischen Saisonverlauf pflegen.",
    model: models.RaceEvent,
    ...
    hidden: true,
    fields: [
      relation("LeagueId", "F1-Liga", models.League, (row) => row.name, true, {
        where: { type: "f1" },
      }),
      text("title", "Rennen", true),
      text("circuit", "Strecke"),
  ...
  lmuCalendar: {
    ...
    group: "Rennkalender",
    description:
      "LMU-Termine pflegen; der nächste veröffentlichte Termin erscheint automatisch auf der Startseite.",
    model: models.RaceEvent,
    getListWhere: listWhereForLeagueType("lmu"),
    hidden: true,
    fields: [
  ...
  lmuStandingImages: {
    title: "LMU WM-Grafiken",
    group: "LMU",
    description: "Legacy-Wertungen für die LMU-Seite hochladen.",
    model: models.LmuStandingImage,
    upload: { field: "imagePath", label: "WM-Grafik", required: true },
    hidden: true,
    fields: [
      relation("LeagueId", "LMU-Liga", models.League, (row) => row.name, true, {
        where: { type: "lmu" },
      ...
      text("season", "Saison", true),
      text("event", "Rennevent"),
      text("title", "Titel", true),
      textarea("description", "Beschreibung"),
      text("altText", "Bildbeschreibung", true, {
        help: "Kurze Beschreibung für Screenreader.",
      }),
      number("sortOrder", "Reihenfolge", false, { min: 0 }),
    ],
  },
  participatingLeagues: {
    ...
    group: "WDL",
    description:
      "WDL-Ligen mit Logo, Link, Aktivstatus und zugeordnetem zentralen Formel-1-Team verwalten.",
    model: models.ParticipatingLeague,
    upload: { field: "logoPath", label: "Liga-Logo" },
    prepareValues: prepareParticipatingLeague,
    afterSave: syncParticipatingLeague,
    fields: [
      text("name", "Liganame", true),
      text("abbreviation", "Kürzel", false, { placeholder: "KRL" }),
      text("constructorName", "Konstrukteur"),
      url("websiteUrl", "Link"),
      checkbox("isActive", "Aktiv"),
      relation(
        "F1TeamId",
        "Zugeordnetes Formel-1-Team",
        models.Team,
        (row) => row.name,
        false,
        { where: { LeagueId: null, discipline: "f1" } },
  ...
  competitionStandings: {
    title: "WDL-Teamstandings",
    group: "Wettkampf der Ligen",
    description:
      "Legacy-Tabelle; neue WDL-Standings werden aus dem Saisonverlauf berechnet.",
    model: models.LeagueCompetitionStanding,
    prepareValues: prepareWdlStanding,
    hidden: true,
    fields: [
      relation(
        "ParticipatingLeagueId",
      ...
      text("constructorName", "Konstrukteur"),
      number("points", "Punkte", true, { min: 0, step: 0.5 }),
      number("wins", "Siege", false, { min: 0, step: 1 }),
      text("gap", "Rückstand"),
      number("sortOrder", "Reihenfolge", false, { min: 0 }),
    ],
  },
  krlTeams: {
    ...
    description:
      "Planungsgruppen für „Unser Team“ mit Farbe und Frontend-Sichtbarkeit pflegen.",
    model: models.KrlTeam,
    prepareValues: prepareKrlTeam,
    nextHref: "/admin/krl-team-planning",
    nextLabel: "Mitglieder grafisch zuordnen",
    fields: [
      text("name", "Name", true),
      field("accentColor", "Farbe", "color", true),
      checkbox("isVisible", "Auf der Startseite anzeigen"),
    ],
  },
  krlTeamAssignments: {
    title: "+ Fahrer-Rollen",
    ...
    model: models.KrlTeamAssignment,
    prepareValues: prepareKrlAssignment,
    upload: { field: "imagePath", label: "Positions-/Personenbild" },
    fields: [
      relation(
        "KrlTeamId",
        "KRL-Team",
      ...
      text("roleName", "Rolle / Funktion", true),
      number("sortOrder", "Reihenfolge", false, { min: 0 }),
    ],
  },
  krlIcons: {
    ...
    group: "KRL Icons",
    description:
      "KRL-Icon aus Fahrer, Beschreibung und Ernennungsmonat pflegen.",
    model: models.KrlIcon,
    prepareValues: prepareKrlIcon,
    prepareEntry: prepareKrlIconForForm,
    fields: [
      relation(
        "DriverId",
        "Fahrer",
      ...
      textarea("text", "Text / Beschreibung", true),
      month("appointedAt", "Ernannt (Monat und Jahr)", false, {
        help: "Auf der Webseite wird ausschließlich Monat und Jahr angezeigt.",
      }),
      number("sortOrder", "Reihenfolge", false, { min: 0 }),
    ],
  },
